...
// .wrangler/tmp/bundle-NAJ01U/checked-fetch.js
...
app2.get("/:id/files/:fileId", async (c) => {
  try {
    const projectId = c.req.param("id");
...
app2.put("/:id/files/:fileId", async (c) => {
  try {
    const projectId = c.req.param("id");
    const fileId = c.req.param("fileId");
    const userId = c.req.header("X-User-ID");
...
var projectRoutes = app2;

// src/worker/routes/collaboration.ts
var app3 = new Hono2();
app3.get("/room/:roomId", async (c) => {
  try {
...
app3.get("/room/:roomId/participants", async (c) => {
  try {
    const roomId = c.req.param("roomId");
    const userId = c.req.header("X-User-ID");
    if (!userId) {
...
app3.post("/invite", async (c) => {
  try {
    ...
    await c.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).bind(projectId, invitee.id, permissions, (/* @__PURE__ */ new Date()).toISOString()).run();
    return c.json({
      success: true,
...
app3.delete("/collaborator/:projectId/:userId", async (c) => {
  try {
...
app3.get("/project/:projectId/collaborators", async (c) => {
  try {
    const projectId = c.req.param("projectId");
    const userId = c.req.header("X-User-ID");
    if (!userId) {
...
app3.put("/collaborator/:projectId/:userId", async (c) => {
  try {
    const projectId = c.req.param("projectId");
    const collaboratorId = c.req.param("userId");
    const userId = c.req.header("X-User-ID");
    ...
    await c.env.DB.prepare(`
      ...
      WHERE project_id = ? AND user_id = ?
    `).bind(permissions, (/* @__PURE__ */ new Date()).toISOString(), projectId, collaboratorId).run();
    return c.json({
      success: true,
...
app3.get("/analytics/:projectId", async (c) => {
  try {
    const projectId = c.req.param("projectId");
    const userId = c.req.header("X-User-ID");
    if (!userId) {
...
var collaborationRoutes = app3;

// src/worker/routes/ai.ts
var app4 = new Hono2();
app4.post("/completion", async (c) => {
  try {
    const body = await c.req.json();
...
// src/worker/routes/deployments.ts
var app5 = new Hono2();
app5.post("/", async (c) => {
  try {
    const userId = c.req.header("X-User-ID");
...
app5.get("/project/:projectId", async (c) => {
  try {
    const projectId = c.req.param("projectId");
    const userId = c.req.header("X-User-ID");
    if (!userId) {
...
async function buildProject(files, assets) {
  try {
    const buildLogs = [];
...
var deploymentRoutes = app5;

// src/worker/routes/analytics.ts
var app6 = new Hono2();
app6.post("/track", async (c) => {
  try {
    const body = await c.req.json();
...
app6.get("/user/:userId", async (c) => {
  try {
    ...
    const eventTypes = results.map((row) => row.type);
...
app6.get("/project/:projectId", async (c) => {
  try {
    const projectId = c.req.param("projectId");
    const days = parseInt(c.req.query("days") || "7");
    if (!projectId) {
    ...
    const totalEvents = results.reduce((sum, row) => sum + row.count, 0);
    const uniqueUsers = [...new Set(results.map((row) => row.user_id))].length;
    const eventTypes = [...new Set(results.map((row) => row.type))];
    return c.json({
      success: true,
      data: {
        projectId,
...
app6.get("/collaboration/:projectId", async (c) => {
  try {
    const projectId = c.req.param("projectId");
    const days = parseInt(c.req.query("days") || "7");
    if (!projectId) {
    ...
    const totalEvents = results.reduce((sum, row) => sum + row.events, 0);
    const totalCursorUpdates = results.reduce((sum, row) => sum + row.cursor_updates, 0);
    const totalCodeChanges = results.reduce((sum, row) => sum + row.code_changes, 0);
    const totalChatMessages = results.reduce((sum, row) => sum + row.chat_messages, 0);
    return c.json({
      success: true,
      data: {
        projectId,
...
app6.get("/export/:projectId", async (c) => {
  try {
    const projectId = c.req.param("projectId");
...
var analyticsRoutes = app6;

// src/worker/durable-objects/collaboration-room.ts
var CollaborationRoom = class {
  static {
    __name(this, "CollaborationRoom");
  }
  ...
  handleCodeChange(userId, change) {
  ...
  applyOperationalTransform(change) {
    const concurrentChanges = this.document.changes.filter(
      (c) => c.timestamp > Date.now() - 5e3
      // Last 5 seconds
    );
  ...
  handlePing(userId) {
    const participant = this.participants.get(userId);
    if (participant) {
      participant.lastSeen = /* @__PURE__ */ new Date();
      this.sendToParticipant(userId, { type: "pong" });
    }
  }
  handleParticipantDisconnect(userId) {
...
app7.route("/api/auth", authRoutes);
app7.route("/api/projects", projectRoutes);
app7.route("/api/collaboration", collaborationRoutes);
app7.route("/api/ai", aiRoutes);
app7.route("/api/deployments", deploymentRoutes);
app7.route("/api/analytics", analyticsRoutes);
app7.get("/ws/:roomId", async (c) => {
  const roomId = c.req.param("roomId");
...
var worker_default = app7;

// ../../../../../../.nvm/versions/node/v22.17.1/lib/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../../../../../.nvm/versions/node/v22.17.1/lib/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-NAJ01U/middleware-insertion-facade.js
...
// ../../../../../../.nvm/versions/node/v22.17.1/lib/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-NAJ01U/middleware-loader.entry.ts
